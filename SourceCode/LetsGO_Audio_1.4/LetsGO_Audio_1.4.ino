/* LetsGO Cue controller. (Basic Version)
   ...
   You must select Board: Teensy-4 and USB-Type: "Serial + Kayboard + MIDI + Audio" from the "Tools menu.

  
   History:
   Version 1.4  / Addapted for Teensy 4 and Audio-Board Rev D. (Based on LetsGO v1.3)
...
const int KEY_STROKE_0[NUM_OF_BUTTONS] = {KEY_SPACE, KEY_ESC, KEY_UP, KEY_DOWN};               // Qlab or GObutton
const int KEY_STROKE_1[NUM_OF_BUTTONS] = {KEY_ENTER, KEY_P, KEY_UP, KEY_DOWN};                 // Ableton Live
...
const int KEY_STROKE_5[NUM_OF_BUTTONS] = {KEY_PAGE_DOWN, KEY_PAGE_UP, KEY_UP, KEY_DOWN};       // PDF Reader
...
void loop()
{
  ...
  if (digitalRead(6) == LOW && digitalRead(9) == LOW && digitalRead(10) == LOW) {
    keyMode = 7;
  }
  else if (digitalRead(9) == LOW && digitalRead(10) == LOW) {
    keyMode = 6;
  }  
  else if (digitalRead(6) == LOW && digitalRead(10) == LOW) {
    keyMode = 5;
  }
